...
- **Over-engineering.** It proposed a per-firm copy of the published-versions table (it holds no client data, so one shared table is enough), an outbox plus an event processor on the firm side (removed: our tables share the firm's database and are written in the same transaction), and a summary reference in the decision log (removed: summary revisions are dated, so the dates already tell which one a decision saw).
- **Invented numbers.** It wrote a p95 latency target and alert thresholds (15 min, 24 h, 7 days, 99%) that the brief never gives. Removed; thresholds are set during the pilot.
- **Observability that measured the wrong thing.** Its first version watched whether components were running (latency, queue age, the citation check passing), not whether they were right. Rewritten around correctness: an end-to-end drift check of pending updates, and content-team grading of summaries for completeness, faithfulness, clarity and neutrality.
- **Scope of the security requirement.** Its first answer covered where the data is stored; re-reading "whatever tracks and surfaces that state" extended isolation to events, backfill, API, logs and metrics.
- **Summary generation.** It proposed deterministic per-change-type rules plus an LLM; I rejected the rules because we do not know the template schema, keeping only the obvious deterministic check (citations and coverage).
- **Decline semantics.** The walk-back flow (decline the latest, get offered the previous version) and the "from" column in the decision history came from me.
...
- **Let invariants check AI-written logic.** The pending-update rule is a pure function with property-based tests; an AI-written change ships only if they pass.
...
- **Whether the design is complete.** It declared decisions closed before the end-to-end flow was worked through, and it used a component in the observability section (the post-publish count) that the architecture never defined. Completeness is checked by walking each flow, not by how finished the text looks.
